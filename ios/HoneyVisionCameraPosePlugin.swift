...
public class HoneyVisionCameraPosePlugin: FrameProcessorPlugin {
  ...
  public override func callback(
    ...
    do {
      ...
      HoneyVisionCameraPosePlugin.timingCounter += 1
      if HoneyVisionCameraPosePlugin.timingCounter % 30 == 0 && !landmarks.isEmpty {
        let inferenceMs = Int((t1 - t0) * 1000)
        let totalMs = Int((t2 - t0) * 1000)
        var first = landmarks[0]
        first["debugInferenceMs"] = inferenceMs
        first["debugTotalMs"] = totalMs
        return [first] + landmarks.dropFirst()
      }

...
exports.scoreSwing = scoreSwing;
function scoreAngle(value, ideal, tolerance) {
    if (value == null)
        return 50;
...
function scoreSwing(params) {
    ...
    const score = Math.max(0, Math.min(100, Math.round(parts.reduce((sum, value) => sum + value, 0) / parts.length)));
    return {
        score,
        honeyBoom: score >= 85,
    };

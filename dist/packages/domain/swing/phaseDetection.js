...
exports.detectSwingPhases = detectSwingPhases;
exports.detectImpact = detectImpact;
exports.getVisiblePhases = getVisiblePhases;
...
function computeVelocities(points) {
    const velocities = [0];
...
function smoothVelocities(velocities, window = 5) {
...
function findMinYIndex(points, startIdx, endIdx) {
    let minY = Infinity;
    let minIdx = startIdx;
...
function findMaxVelocityIndex(velocities, startIdx, endIdx) {
    let maxV = 0;
    let maxIdx = startIdx;
    for (let i = startIdx; i <= endIdx; i++) {
...
function fallbackPhases(points) {
...
function detectSwingPhases(points) {
    if (points.length < 6) {
        return [];
    }
...
function tryHeuristicDetection(points) {
...
function getVisiblePhases(phases, currentTimeMs) {

...
1. Camera-denied dead-end on the core screen (`app/(tabs)/record.tsx:491`) — P1.1
2. Reachable crash paths (`app/clinic/physical-check.tsx:49`, `app/clinic/retention.tsx:19`) — P1.3
...
1. **Camera-denied dead-end on the core screen** — `app/(tabs)/record.tsx:491-496` renders only a spinner + "Camera permission denied", no Open Settings / retry. `Linking.openSettings()` appears nowhere in the codebase. Beta review exercises the main flow; a reviewer who taps "Don't Allow" dead-ends the app's core feature. Fix: add an Open Settings button (also to `app/grip/capture.tsx:245-255` siblings, which have only "Go Back").
...
3. **Deep-linkable dev/orphaned routes ship in the bundle** —
   ...
   - `app/clinic/*` coach/debug dashboards (`imu-debug.tsx` writes files to `documentDirectory`, `dave-dashboard.tsx`, phase inspectors) are gated only at the Settings entry (`isCoach`), not inside the screens — deep links bypass the gate.
   - `app/clinic/physical-check.tsx:49` and `app/clinic/retention.tsx:19` contain reachable `throw new Error('Not implemented')` → crash paths.
...
1. **App-authored `PrivacyInfo.xcprivacy` missing.** Pod aggregation is enabled (`Podfile.properties.json` `apple.privacyManifestAggregationEnabled: "true"`) so SDK manifests (MediaPipe 0.10.33, RevenueCat 5.67.1, GoogleSignIn-via-Clerk) aggregate at build, but the app's own data-collection declaration and the ASC privacy nutrition labels must cover: swings/pose/video uploaded to Supabase, first-party `events` telemetry (`lib/eventBus.ts:186`), Clerk identity, RevenueCat purchase data, and **kid profiles in clinic mode** (`lib/clinic/kidProfileStore.ts` — minors' data).

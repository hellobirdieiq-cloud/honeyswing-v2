...
public class HoneyVisionCameraPosePlugin: FrameProcessorPlugin {
  ...
  /// Copy a CVPixelBuffer so we don't retain the camera session's pool buffer.
  /// Holding pool buffers starves the AVCaptureVideoDataOutput and stops frame delivery.
  private static func copyPixelBuffer(_ src: CVPixelBuffer) -> CVPixelBuffer? {
    ...
    let status = CVPixelBufferCreate(kCFAllocatorDefault, width, height, fmt, nil, &dst)
    ...
    if srcBPR == dstBPR {
      ...
      for row in 0..<height {
        memcpy(d.advanced(by: row * dstBPR), s.advanced(by: row * srcBPR), copyBPR)
      }
    }
    return dest
  }
  ...
  private static func stashGripFrame(pixelBuffer: CVPixelBuffer, timestamp: Double) {
    ...
    gripBuffer[idx] = (buffer: copy, timestamp: timestamp)
